...
import com.missile.fastadapter.refresh.footer.BallPulseFooter;
import com.missile.fastadapter.refresh.footer.ClassicsFooter;
import com.missile.fastadapter.refresh.header.BezierRadarHeader;
import com.missile.fastadapter.refresh.impl.RefreshContentWrapper;
import com.missile.fastadapter.refresh.impl.RefreshFooterWrapper;
import com.missile.fastadapter.refresh.impl.RefreshHeaderWrapper;
import com.missile.fastadapter.refresh.api.DefaultRefreshFooterCreator;
import com.missile.fastadapter.refresh.api.DefaultRefreshHeaderCreator;
import com.missile.fastadapter.refresh.api.DefaultRefreshInitializer;
import com.missile.fastadapter.refresh.api.RefreshContent;
import com.missile.fastadapter.refresh.api.RefreshFooter;
import com.missile.fastadapter.refresh.api.RefreshHeader;
import com.missile.fastadapter.refresh.api.RefreshInternal;
import com.missile.fastadapter.refresh.api.RefreshKernel;
...
import com.missile.fastadapter.refresh.listener.OnMultiPurposeListener;
import com.missile.fastadapter.refresh.listener.OnRefreshListener;
import com.missile.fastadapter.refresh.listener.OnRefreshLoadMoreListener;
import com.missile.fastadapter.refresh.listener.OnStateChangedListener;
import com.missile.fastadapter.refresh.util.DensityUtil;
import com.missile.fastadapter.refresh.util.SmartUtil;
...
public class AgileRefreshLayout extends ViewGroup implements RefreshLayout, NestedScrollingParent {
    ...
    protected int mSpinner;//当前的spinner
    protected int mLastSpinner;//最后的spinner
    protected int mTouchSpinner;//触摸时候的spinner
    protected int mScreenHeightPixels; //屏幕高度
    protected int mFloorDuration = 250;
    protected int mReboundDuration = 250;
    protected float mTouchX;
    protected float mTouchY;
    protected float mLastTouchX;//用于实现Header的左右拖到效果
    protected float mLastTouchY;//实现多点触摸
    ...
    protected boolean mEnableClipHeaderWhenFixedBehind = true; //当Header FixedBehind 时候是否剪裁遮挡Header
    protected boolean mEnableClipFooterWhenFixedBehind = true; //当Footer FixedBehind 时候是否剪裁遮挡Footer
    ...
    protected boolean mEnableFooterFollowWhenLoadFinished = false;//是否在全部加载结束之后Footer跟随内容
    protected boolean mEnablePreviewInEditMode = true; //是否在编辑模式下开启预览功能
    protected boolean mEnableOverScrollBounce = true; //是否启用越界回弹
    protected boolean mEnableOverScrollDrag = false; // 是否启用越界拖动
    ...
    protected boolean mDisableContentWhenRefresh = false;//是否开启在刷新的时候禁止操作内容视图
    protected boolean mDisableContentWhenLoading = false;//是否开启在加载的时候禁止操作内容视图
    protected boolean mFooterNoMoreData = false;//数据是否全部加载完成，如果完成就不能再触发加载事件
    ...
    protected float mFooterTriggerRate = 1.0f; //触发加载距离与FooterHeight 的比率
    ...
    protected RefreshState mViceState = RefreshState.None;//主状态刷新时候的滚动状态

    protected long mLastOpenTime = 0;//上一次刷新或者加载的时间
    ...
    public AgileRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mReboundDuration = ta.getInt(R.styleable.AgileRefreshLayout_srlReboundDuration, mReboundDuration);
        mHeaderInsetStart = ta.getDimensionPixelOffset(R.styleable.AgileRefreshLayout_srlHeaderInsetStart, mHeaderInsetStart);
        mFooterInsetStart = ta.getDimensionPixelOffset(R.styleable.AgileRefreshLayout_srlFooterInsetStart, mFooterInsetStart);
        mNestedChild.setNestedScrollingEnabled(ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableNestedScrolling, mNestedChild.isNestedScrollingEnabled()));
        mDragRate = ta.getFloat(R.styleable.AgileRefreshLayout_srlDragRate, mDragRate);
        mHeaderMaxDragRate = ta.getFloat(R.styleable.AgileRefreshLayout_srlHeaderMaxDragRate, mHeaderMaxDragRate);
        mFooterMaxDragRate = ta.getFloat(R.styleable.AgileRefreshLayout_srlFooterMaxDragRate, mFooterMaxDragRate);
        mHeaderTriggerRate = ta.getFloat(R.styleable.AgileRefreshLayout_srlHeaderTriggerRate, mHeaderTriggerRate);
        mFooterTriggerRate = ta.getFloat(R.styleable.AgileRefreshLayout_srlFooterTriggerRate, mFooterTriggerRate);
        mEnableRefresh = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableRefresh, mEnableRefresh);
        mEnableLoadMore = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableLoadMore, mEnableLoadMore);
        mEnableHeaderTranslationContent = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableHeaderTranslationContent, mEnableHeaderTranslationContent);
        mEnableFooterTranslationContent = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableFooterTranslationContent, mEnableFooterTranslationContent);
        mHeaderTranslationViewId = ta.getResourceId(R.styleable.AgileRefreshLayout_srlHeaderTranslationViewId, mHeaderTranslationViewId);
        mFooterTranslationViewId = ta.getResourceId(R.styleable.AgileRefreshLayout_srlFooterTranslationViewId, mFooterTranslationViewId);
        mEnablePreviewInEditMode = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnablePreviewInEditMode, mEnablePreviewInEditMode);
        mEnableAutoLoadMore = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableAutoLoadMore, mEnableAutoLoadMore);
        mEnableOverScrollBounce = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableOverScrollBounce, mEnableOverScrollBounce);
        mEnablePureScrollMode = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnablePureScrollMode, mEnablePureScrollMode);
        mEnableScrollContentWhenLoaded = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableScrollContentWhenLoaded, mEnableScrollContentWhenLoaded);
        mEnableScrollContentWhenRefreshed = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableScrollContentWhenRefreshed, mEnableScrollContentWhenRefreshed);
        mEnableLoadMoreWhenContentNotFull = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableLoadMoreWhenContentNotFull, mEnableLoadMoreWhenContentNotFull);
        mEnableFooterFollowWhenLoadFinished = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableFooterFollowWhenLoadFinished, mEnableFooterFollowWhenLoadFinished);
        mEnableClipHeaderWhenFixedBehind = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableClipHeaderWhenFixedBehind, mEnableClipHeaderWhenFixedBehind);
        mEnableClipFooterWhenFixedBehind = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableClipFooterWhenFixedBehind, mEnableClipFooterWhenFixedBehind);
        mEnableOverScrollDrag = ta.getBoolean(R.styleable.AgileRefreshLayout_srlEnableOverScrollDrag, mEnableOverScrollDrag);
        mDisableContentWhenRefresh = ta.getBoolean(R.styleable.AgileRefreshLayout_srlDisableContentWhenRefresh, mDisableContentWhenRefresh);
        mDisableContentWhenLoading = ta.getBoolean(R.styleable.AgileRefreshLayout_srlDisableContentWhenLoading, mDisableContentWhenLoading);
        mFixedHeaderViewId = ta.getResourceId(R.styleable.AgileRefreshLayout_srlFixedHeaderViewId, mFixedHeaderViewId);
        mFixedFooterViewId = ta.getResourceId(R.styleable.AgileRefreshLayout_srlFixedFooterViewId, mFixedFooterViewId);

        if (mEnablePureScrollMode && !ta.hasValue(R.styleable.AgileRefreshLayout_srlEnableOverScrollDrag)) {
            mEnableOverScrollDrag = true;
        }
        mManualLoadMore = mManualLoadMore || ta.hasValue(R.styleable.AgileRefreshLayout_srlEnableLoadMore);
        mManualHeaderTranslationContent = mManualHeaderTranslationContent || ta.hasValue(R.styleable.AgileRefreshLayout_srlEnableHeaderTranslationContent);
        mManualFooterTranslationContent = mManualFooterTranslationContent || ta.hasValue(R.styleable.AgileRefreshLayout_srlEnableFooterTranslationContent);
        mManualNestedScrolling = mManualNestedScrolling || ta.hasValue(R.styleable.AgileRefreshLayout_srlEnableNestedScrolling);
        mHeaderHeightStatus = ta.hasValue(R.styleable.AgileRefreshLayout_srlHeaderHeight) ? DimensionStatus.XmlLayoutUnNotify : mHeaderHeightStatus;
        mFooterHeightStatus = ta.hasValue(R.styleable.AgileRefreshLayout_srlFooterHeight) ? DimensionStatus.XmlLayoutUnNotify : mFooterHeightStatus;
        int accentColor = ta.getColor(R.styleable.AgileRefreshLayout_srlAccentColor, 0);
        int primaryColor = ta.getColor(R.styleable.AgileRefreshLayout_srlPrimaryColor, 0);
        if (primaryColor != 0) {
            if (accentColor != 0) {
        ...
        if (!thisView.isInEditMode()) {
            ...
            if (!mManualNestedScrolling && !isNestedScrollingEnabled()) {
                post(new Runnable() {
                    @Override
                    public void run() {
                        final View thisView = AgileRefreshLayout.this;
                        for (ViewParent parent = thisView.getParent(); parent != null; ) {
                            if (parent instanceof NestedScrollingParent) {
                                View target = AgileRefreshLayout.this;
                ...
                if (!mIsBeingDragged && mDragDirection != 'h' && mRefreshContent != null) {//没有拖动之前，检测  canRefresh canLoadMore 来开启拖动
                    if (mDragDirection == 'v' || (Math.abs(dy) >= mTouchSlop && Math.abs(dx) < Math.abs(dy))) {//滑动允许最大角度为45度
                        ...
                        if (mIsBeingDragged) {
                            ...
                            if (parent != null) {
                                //修复问题 https://github.com/scwang90/AgileRefreshLayout/issues/580
        ...
        AnimatorListenerAdapter listener = new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (mRefreshListener != null) {
                    mRefreshListener.onRefresh(AgileRefreshLayout.this);
                } else if (mOnMultiPurposeListener == null) {
                    finishRefresh(3000);
                }
                if (mRefreshHeader != null) {
                    mRefreshHeader.onStartAnimator(AgileRefreshLayout.this, mHeaderHeight, (int) (mHeaderMaxDragRate * mHeaderHeight));
                }
                if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    mOnMultiPurposeListener.onRefresh(AgileRefreshLayout.this);
...
//            reboundAnimator.setDuration(20000);
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            super(context, attrs);
            TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.AgileRefreshLayout_Layout);
            backgroundColor = ta.getColor(R.styleable.AgileRefreshLayout_Layout_layout_srlBackgroundColor, backgroundColor);
            if (ta.hasValue(R.styleable.AgileRefreshLayout_Layout_layout_srlSpinnerStyle)) {
                spinnerStyle = SpinnerStyle.values()[ta.getInt(R.styleable.AgileRefreshLayout_Layout_layout_srlSpinnerStyle, SpinnerStyle.Translate.ordinal())];
            }
            ta.recycle();
        }
    ...
    @Override
    public AgileRefreshLayout setFooterHeight(float heightDp) {
    ...
    @Override
    public AgileRefreshLayout setHeaderHeight(float heightDp) {
    ...
    @Override
    public AgileRefreshLayout setHeaderInsetStart(float insetDp) {
        mHeaderInsetStart = DensityUtil.dp2px(insetDp);
        return this;
    }

    @Override
    public AgileRefreshLayout setFooterInsetStart(float insetDp) {
     ...
     */
    @Override
    public AgileRefreshLayout setDragRate(float rate) {
        this.mDragRate = rate;
        return this;
    }
     ...
     */
    @Override
    public AgileRefreshLayout setHeaderMaxDragRate(float rate) {
     ...
     * @return AgileRefreshLayout
     */
    @Override
    public AgileRefreshLayout setFooterMaxDragRate(float rate) {
     ...
     */
    @Override
    public AgileRefreshLayout setHeaderTriggerRate(float rate) {
     ...
     */
    @Override
    public AgileRefreshLayout setFooterTriggerRate(float rate) {
     ...
     */
    @Override
    public AgileRefreshLayout setReboundInterpolator(@NonNull Interpolator interpolator) {
     ...
     */
    @Override
    public AgileRefreshLayout setReboundDuration(int duration) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableLoadMore(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableRefresh(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableHeaderTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableFooterTranslationContent(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableAutoLoadMore(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableOverScrollBounce(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnablePureScrollMode(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableScrollContentWhenLoaded(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableScrollContentWhenRefreshed(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableLoadMoreWhenContentNotFull(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableOverScrollDrag(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableFooterFollowWhenLoadFinished(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableClipHeaderWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setEnableClipFooterWhenFixedBehind(boolean enabled) {
     ...
     */
    @Override
    public AgileRefreshLayout setDisableContentWhenRefresh(boolean disable) {
     ...
     */
    @Override
    public AgileRefreshLayout setDisableContentWhenLoading(boolean disable) {
     ...
     */
    @Override
    public AgileRefreshLayout setRefreshHeader(@NonNull RefreshHeader header) {
     ...
     */
    @Override
    public AgileRefreshLayout setRefreshHeader(@NonNull RefreshHeader header, int width, int height) {
        if (mRefreshHeader != null) {
     ...
     */
    @Override
    public AgileRefreshLayout setRefreshFooter(@NonNull RefreshFooter footer) {
     ...
     */
    @Override
    public AgileRefreshLayout setRefreshFooter(@NonNull RefreshFooter footer, int width, int height) {
        if (mRefreshFooter != null) {
     ...
     */
    @Override
    public AgileRefreshLayout getLayout() {
        return this;
    }
     ...
     */
    @Override
    public AgileRefreshLayout setOnRefreshListener(OnRefreshListener listener) {
     ...
     */
    @Override
    public AgileRefreshLayout setOnLoadMoreListener(OnLoadMoreListener listener) {
     ...
     */
    @Override
    public AgileRefreshLayout setOnRefreshLoadMoreListener(OnRefreshLoadMoreListener listener) {
        this.mRefreshListener = listener;
        this.mLoadMoreListener = listener;
        this.mEnableLoadMore = mEnableLoadMore || (!mManualLoadMore && listener != null);
        return this;
     ...
     */
    @Override
    public AgileRefreshLayout setOnMultiPurposeListener(OnMultiPurposeListener listener) {
     ...
     */
    @Override
    public AgileRefreshLayout setPrimaryColors(@ColorInt int... primaryColors) {
        if (mRefreshHeader != null) {
     ...
     */
    @Override
    public AgileRefreshLayout setPrimaryColorsId(@ColorRes int... primaryColorId) {
        final View thisView = this;
     ...
     */
    @Override
    public AgileRefreshLayout setScrollBoundaryDecider(ScrollBoundaryDecider boundary) {
     ...
     */
    @Override
    public AgileRefreshLayout setNoMoreData(boolean noMoreData) {
     ...
     */
    @Override
    public AgileRefreshLayout finishRefresh() {
     ...
     */
    @Override
    public AgileRefreshLayout finishLoadMore() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public AgileRefreshLayout finishRefresh(int delayed) {
     ...
     */
    @Override
    public AgileRefreshLayout finishRefresh(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public AgileRefreshLayout finishRefresh(int delayed, final boolean success) {
        ...
        postDelayed(new Runnable() {
            ...
            public void run() {
                if (mState == RefreshState.Refreshing && mRefreshHeader != null && mRefreshContent != null) {
                    ...
                    int startDelay = mRefreshHeader.onFinish(AgileRefreshLayout.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshHeader instanceof RefreshHeader) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            AgileRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + mSpinner - mTouchSlop * 2, 0));
                            AgileRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + mSpinner, 0));
     ...
     */
    @Override
    public AgileRefreshLayout finishLoadMore(int delayed) {
     ...
     */
    @Override
    public AgileRefreshLayout finishLoadMore(boolean success) {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
     ...
     */
    @Override
    public AgileRefreshLayout finishLoadMore(int delayed, final boolean success, final boolean noMoreData) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
                if (mState == RefreshState.Loading && mRefreshFooter != null && mRefreshContent != null) {
                    notifyStateChanged(RefreshState.LoadFinish);
                    final int startDelay = mRefreshFooter.onFinish(AgileRefreshLayout.this, success);
                    if (mOnMultiPurposeListener != null && mRefreshFooter instanceof RefreshFooter) {
                    ...
                    if (startDelay < Integer.MAX_VALUE) {
                        ...
                        if (mIsBeingDragged || mNestedInProgress) {
                            ...
                            AgileRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_DOWN, mLastTouchX, mLastTouchY + offset + mTouchSlop * 2, 0));
                            AgileRefreshLayout.super.dispatchTouchEvent(obtain(time, time, MotionEvent.ACTION_MOVE, mLastTouchX, mLastTouchY + offset, 0));
                            if (mNestedInProgress) {
                                mTotalUnconsumed = 0;
                            }
                        }
     ...
     */
    @Override
    public AgileRefreshLayout finishLoadMoreWithNoMoreData() {
        long passTime = System.currentTimeMillis() - mLastOpenTime;
    ...
    public class RefreshKernelImpl implements RefreshKernel {
        ...
        public RefreshLayout getRefreshLayout() {
            return AgileRefreshLayout.this;
        }
        ...
        public ValueAnimator animSpinner(int endSpinner) {
            return AgileRefreshLayout.this.animSpinner(endSpinner, 0, mReboundInterpolator, mReboundDuration);
        }
    ...
    public class ViscousFluidInterpolator implements Interpolator {
        ...
        private final float VISCOUS_FLUID_NORMALIZE = 1.0f / viscousFluid(1.0f);
        private final float VISCOUS_FLUID_OFFSET = 1.0f - VISCOUS_FLUID_NORMALIZE * viscousFluid(1.0f);
        ;
